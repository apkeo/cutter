...
async function main(){
 ...
 const page=await require('../tests/app-window.cjs')(app);
 try{
 await app.evaluate(({BrowserWindow})=>{const main=BrowserWindow.getAllWindows().find(w => w.webContents.getURL().includes('page=index'));main.setSize(1440,1000);});
 ...
 const media=await page.evaluate(file=>window.cutter.importFile(file),clip);await app.evaluate(({BrowserWindow},media)=>BrowserWindow.getAllWindows().find(w => w.webContents.getURL().includes('page=index')).webContents.send('media',media),media);
