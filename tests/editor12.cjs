...
(async()=>{
 ...
 try{
  ...
  const output=await page.locator('#export-filename').textContent();const raw=execFileSync(ffmpeg,['-v','error','-i',output,'-f','rawvideo','-pix_fmt','rgb24','pipe:1'],{windowsHide:true,maxBuffer:4e6});
  assert.deepEqual([...raw.subarray(0,3)],[0,0,0],'Square top-left corner preserves source');assert.ok([...raw.subarray(639*3,639*3+3)].every((v,i)=>Math.abs(v-[215,227,209][i])<=2),'Rounded top-right reveals background');
