...
const {sampleAt,cursorAt,smoothPoints}=require('../dist/electron/cursor.cjs');
test('cursor interpolation preserves precise click anchors and shape transitions',()=>{const track={smooth:true,samples:[{t:0,x:0,y:0,shape:'arrow'},{t:1,x:1,y:1,shape:'hand'}],clicks:[{t:.5,end:.8,x:.4,y:.6,button:'left'}]};assert.equal(sampleAt(track.samples,1).shape,'hand');const pose=cursorAt(track,.5,smoothPoints(track));assert.equal(pose.x,.4);assert.equal(pose.y,.6);assert.equal(sampleAt(track.samples,.25).x,.25);});
